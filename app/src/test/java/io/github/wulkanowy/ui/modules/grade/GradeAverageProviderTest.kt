...
class GradeAverageProviderTest {
    ...
    fun `calc current semester standard average with no weights`() {
        ...
        } returns flowWithResource { noWeightGrades to noWeightGradesSummary }
    ...
    fun `calc current semester arithmetic average with no weights`() {
        ...
        } returns flowWithResource { noWeightGrades to noWeightGradesArithmeticSummary }
    ...
    fun `calc current semester average with load from cache sequence`() {
        ...
        assertEquals(
            3.5,
            items[1].data?.single { it.subject == "Język polski" }!!.average,
            .0
    ...
    fun `calc current semester average`() {
        ...
        } returns flowWithResource { secondGrades to secondSummaries }
    ...
    fun `force calc full year average when current is first`() {
        ...
        } returns flowWithResource { firstGrades to firstSummaries }
    ...
    fun `calc full year average when current is first with load from cache sequence`() {
        ...
        assertEquals(
            3.5,
            items[2].data!!.single { it.subject == "Matematyka" }.average,
            .0
        ) // (from summary): 3,5
        assertEquals(
            3.5,
            items[2].data!!.single { it.subject == "Fizyka" }.average,
            .0
        ) // (from summary): 3,5
    }
    ...
    fun `calc both semesters average when missing summary in second semester`() {
        ...
        } returns flowWithResource { secondGrades to secondSummaries.dropLast(1) }
    ...
    fun `calc both semesters average when missing summary in first semester`() {
        ...
        } returns flowWithResource { firstGrades to firstSummaries.dropLast(1) }
        coEvery {
            gradeRepository.getGrades(
                student,
                semesters[2],
