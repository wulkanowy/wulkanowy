...
class GetAttendanceCalculatorDataUseCase @Inject constructor(
    ...
    operator fun invoke(
    ...
    ): Flow<Resource<List<AttendanceData>>> = combine(
    ...
    ) { targetFreq, subjectsResource ->
        subjectsResource.mapData { subjects -> targetFreq to subjects }
    }.flatMapResourceData { (targetFreq, subjects) ->
        combineResourceFlows(subjects.map { subject ->
            attendanceSummaryRepository.getAttendanceSummary(
                ...
                val presences = summaries.sumOf { it.allPresences }
                val absences = summaries.sumOf { it.allAbsences }
                AttendanceData(
