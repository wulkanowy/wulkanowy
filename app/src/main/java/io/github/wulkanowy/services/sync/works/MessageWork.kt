...
class MessageWork @Inject constructor(
    ...
    private fun notify(messages: List<Message>) {
        notificationManager.notify(Random.nextInt(Int.MAX_VALUE), NotificationCompat.Builder(context, NewEntriesChannel.CHANNEL_ID)
            ...
            .setContentIntent(
                PendingIntent.getActivity(context, FragmentEnum.MESSAGE.ordinal, MainActivity.getStartIntent(context)
                    .putExtra(EXTRA_START_MENU_INDEX, 4)
                    .putExtra(EXTRA_START_MENU_FRAGMENT, FragmentEnum.MESSAGE)
                    , FLAG_UPDATE_CURRENT)
            )
