...
import io.github.wulkanowy.database.dao.entities.DayDao;
import io.github.wulkanowy.database.dao.entities.Lesson;
import io.github.wulkanowy.database.dao.entities.LessonDao;
import io.github.wulkanowy.database.dao.entities.WeekDao;
import io.github.wulkanowy.services.jobs.VulcanJobHelper;
import io.github.wulkanowy.utilities.DataObjectConverter;
...
public class TimetableSync {
    ...
    public void sync(@NonNull LoginSession loginSession, @Nullable String dateOfMonday) throws NotLoggedInErrorException,
        ...
        Query<io.github.wulkanowy.database.dao.entities.Week> weekQuery = weekDao.queryBuilder()
                .where(WeekDao.Properties.UserId.eq(loginSession.getUserId()), WeekDao.Properties.StartDayDate.eq(week.getStartDayDate())).build();

        io.github.wulkanowy.database.dao.entities.Week week1 = weekQuery.unique();
    ...
    private List<Lesson> getPreparedLessonsList(List<Day> dayList, DayDao dayDao, LessonDao lessonDao, long userId, long weekId) {
        ...
        for (Day day : dayList) {
            ...
            Query<io.github.wulkanowy.database.dao.entities.Day> dayQuery = dayDao.queryBuilder()
    ...
    private List<io.github.wulkanowy.database.dao.entities.Day> getPreparedDaysList(List<Day> dayList, long userId, long weekId, DayDao dayDao) {
        List<io.github.wulkanowy.database.dao.entities.Day> updatedDayList = new ArrayList<>();
        List<io.github.wulkanowy.database.dao.entities.Day> dayEntityList = DataObjectConverter
                .daysToDaysEntities(dayList);
        for (io.github.wulkanowy.database.dao.entities.Day day : dayEntityList) {

            Query<io.github.wulkanowy.database.dao.entities.Day> dayQuery = dayDao.queryBuilder().where(DayDao.Properties.UserId.eq(userId), DayDao.Properties.WeekId.eq(weekId), DayDao.Properties.Date.eq(day.getDate())).build();

            io.github.wulkanowy.database.dao.entities.Day day1 = dayQuery.unique();
