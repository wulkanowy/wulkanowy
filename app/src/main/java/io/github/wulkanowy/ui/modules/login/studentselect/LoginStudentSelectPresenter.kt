    ...
    private fun registerStudents(students: List<Student>) {
        disposable.add(studentRepository.saveStudents(students)
            ...
            .subscribe({
                ...
                students.forEach { analytics.logEvent("registration_student_select", "success" to false, "scrapperBaseUrl" to it.scrapperBaseUrl, "symbol" to it.symbol, "error" to error.localizedMessage.ifEmpty { "No message" }) }
